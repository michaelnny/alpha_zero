"""Human vs. AlphaZero on free-style Gomoku game with a small sized board."""
...
def main(argv):
    ...
    network = AlphaZeroNet(input_shape, num_actions, FLAGS.num_res_blocks, FLAGS.num_planes, FLAGS.num_fc_units).to(
        device=runtime_device
    )

    if FLAGS.load_checkpoint_file and os.path.isfile(FLAGS.load_checkpoint_file):
        loaded_state = load_checkpoint(FLAGS.load_checkpoint_file, runtime_device)
        network.load_state_dict(loaded_state['network'])

    network.eval()

    def alpha_zero_player(env: GomokuEnv) -> int:
        player = create_mcts_player(network, runtime_device)
        action, _, _ = player(env, None, 5.0, FLAGS.temp_value, FLAGS.num_simulations, False, True)
        return action

    game_gui = BoardGameGui(eval_env, black_player='human', white_player=alpha_zero_player, show_step=False)
    game_gui.start()
