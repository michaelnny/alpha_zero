...
In practice, we often run the actors on CPU and leaner agent on GPU, with a `1:10` as the ratio for the sample generation rate vs. train sample rate. We figured this imbalanced ratio is reasonable since we're using experience replay to store large amount of samples, and we also argument the samples through random rotation and mirroring during training.
